...
export interface CrewActiveFlag {
  id: string;
  summit_id: string;
...
export async function fetchCrewActiveFlags(crewId: string): Promise<CrewActiveFlag[]> {
  const { data, error } = await supabase
    .from('flags')
    .select('id, summit_id, planted_at, expires_at, summits(name_ko, name_en, name_ja, elevation_m), users(display_name)')
    .eq('crew_id', crewId)
    .eq('is_active', true)
  ...
  return (data ?? []).map((row: any) => ({
    ...
    planted_at: row.planted_at,
    expires_at: row.expires_at,
    planted_by: row.users?.display_name ?? null,
  }));
}
